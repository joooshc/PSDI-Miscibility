...
def feature_scaling(dataset_selected, scaling_method, min_val_dict=None):
    ...
    for feature in to_scale:
        ...
        elif scaling_method == 'squareroot':
            if dataset_selected[feature].min() >= 0:
                dataset_selected[feature] = np.sqrt(dataset_selected[feature])
            else:
                print(f"Can't apply square root to {feature} due to negative values.")
...
if __name__ == '__main__':
    ...
    dataset_scaled, dropped_rows_dict, min_val_dict = feature_scaling(dataset_selected, 'minmax', min_val_dict)
    # dataset_scaled, _, min_val_dict = feature_scaling(dataset_scaled, 'quantile', min_val_dict)
    # dataset_scaled, _, min_val_dict = feature_scaling(dataset_scaled, 'log', min_val_dict) 
    # dataset_scaled, _, min_val_dict = feature_scaling(dataset_scaled, 'minmax', min_val_dict)
    ...
    dataset_scaled.to_csv("method2_dataset_(minmax).csv", index=False)
