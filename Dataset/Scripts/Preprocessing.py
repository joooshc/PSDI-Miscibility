...
if __name__ == '__main__':
    ...
    '''
    dataset_scaled, dropped_rows_dict = feature_scaling(dataset_selected, 'normalize')
    dataset_scaled, _ = feature_scaling(dataset_scaled, 'quantile')
    dataset_scaled, _ = feature_scaling(dataset_selected, 'log') 
    dataset_scaled, _ = feature_scaling(dataset_scaled, 'minmax')
    '''
    dataset_scaled, _, min_val_dict = feature_scaling(dataset_selected, 'log')
    pd.DataFrame(list(min_val_dict.items()), columns=['Feature', 'MinValue']).to_csv('min_vals.csv', index=False)
    
    # For the validation dataset
    '''
    val_scaled, _ = feature_scaling(validation_set, 'normalize')
    val_scaled, _ = feature_scaling(val_scaled, 'quantile')
    val_scaled, _ = feature_scaling(validation_set, 'log') 
    val_scaled, _ = feature_scaling(val_scaled, 'minmax')
    '''
    val_scaled, _, _ = feature_scaling(validation_set, 'log', min_val_dict) 
    val_scaled.to_csv("master_validation_set.csv", index=False)

    y1 = scale_y(dataset_scaled.iloc[:, 5])
    y2 = scale_y(dataset_scaled.iloc[:, 6])
    ...
    dataset_scaled.to_csv("master_dataset_diff_prcd.csv", index=False)
